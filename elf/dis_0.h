...
const char * dis_04(const char * code, unsigned prefix)
{
	...
	{
      	code = mod_reg_rm_ops( code, prefix, OpRegs::YMM0, 0, op1, op2 );
    	printf( "vpmaddubsw %s,%%ymm%d,%s\n", op2.c_str(), vvvv, op1.c_str() );
	}
	else
	{
    	code = mod_reg_rm_ops( code, prefix, OpRegs::XMM0, 0, op1, op2 );
    	printf( "vpmaddubsw %s,%%xmm%d,%s\n", op2.c_str(), vvvv, op1.c_str() );
	}
	return code;
}
...
const char * dis_07(const char * code, unsigned prefix)
{
	...
	{
      	code = mod_reg_rm_ops( code, prefix, OpRegs::YMM0, 0, op1, op2 );
    	printf( "vphsubsw %s,%%ymm%d,%s\n", op2.c_str(), vvvv, op1.c_str() );
	}
	else
	{
    	code = mod_reg_rm_ops( code, prefix, OpRegs::XMM0, 0, op1, op2 );
    	printf( "vphsubsw %s,%%xmm%d,%s\n", op2.c_str(), vvvv, op1.c_str() );
	}
	return code;
}
...
const char * dis_09(const char * code, unsigned prefix)
{
	...
	if( prefix & VEX )
	{
		...
		{
       		code = mod_reg_rm_ops( code, prefix, OpRegs::YMM0, 0, op1, op2 );
	    	printf( "vpsignw %s,%%ymm%d,%s\n", op2.c_str(), vvvv, op1.c_str() );
		}
		else
		{
       		code = mod_reg_rm_ops( code, prefix, OpRegs::XMM0, 0, op1, op2 );
	    	printf( "vpsignw %s,%%xmm%d,%s\n", op2.c_str(), vvvv, op1.c_str() );
		}
	}
	else
	{
...
const char * dis_0a(const char * code, unsigned prefix)
{
	...
	if( prefix & VEX )
	{
		...
		else
		{
			...
			{
   	    		code = mod_reg_rm_ops( code, prefix, OpRegs::YMM0, 0, op1, op2 );
		    	printf( "vpsignd %s,%%ymm%d,%s\n", op2.c_str(), vvvv, op1.c_str() );
			}
			else
			{
   	    		code = mod_reg_rm_ops( code, prefix, OpRegs::XMM0, 0, op1, op2 );
		    	printf( "vpsignd %s,%%xmm%d,%s\n", op2.c_str(), vvvv, op1.c_str() );
			}
		}
	}
	else
...
const char * dis_0f(const char * code, unsigned prefix)
{
	...
	case 0x38:
	{
		...
		else if( code[1] == 0xffffffdb )
		{
			code += 2;
			code = mod_reg_rm_ops( code, prefix, OpRegs::XMM0, 0, op2, op1 );
			printf( "aesimc %s,%s\n", op1.c_str(), op2.c_str() );
		}
		else if( code[1] == 0xffffffdc )
		{
			code += 2;
			code = mod_reg_rm_ops( code, prefix, OpRegs::XMM0, 0, op2, op1 );
			printf( "aesenc %s,%s\n", op1.c_str(), op2.c_str() );
		}
		else if( code[1] == 0xffffffdd )
		{
			code += 2;
			code = mod_reg_rm_ops( code, prefix, OpRegs::XMM0, 0, op2, op1 );
			printf( "aesenclast %s,%s\n", op1.c_str(), op2.c_str() );
		}
		else if( code[1] == 0xffffffde )
		{
			code += 2;
			code = mod_reg_rm_ops( code, prefix, OpRegs::XMM0, 0, op2, op1 );
			printf( "aesdec %s,%s\n", op1.c_str(), op2.c_str() );
